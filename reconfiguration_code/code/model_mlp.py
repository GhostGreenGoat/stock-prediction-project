...
class CustomQuantileLoss(nn.Module):
    def __init__(self, k=5):
        super(CustomQuantileLoss, self).__init__()
        self.num_classes = k
        self.device = torch.device("cuda:2" if torch.cuda.is_available() else "cpu")

    def forward(self, f_x, y):
        f_x = f_x.to(self.device).float()
        y = y.to(self.device).float()
        """
        rank_f_x = f_x.argsort().argsort()  # Rank of each prediction
        normalized_rank_f_x = rank_f_x / (len(rank_f_x) - 1)  # Normalized rank of each prediction
        rank_y = y.argsort().argsort()  
        normalized_rank_y = rank_y / (len(rank_y) - 1)  # Normalized rank of each target
        """

        quantiles = torch.quantile(y, torch.linspace(0, 1, steps=self.num_classes+1,device=self.device)).to(self.device)
        #去掉quantiles的第一位和最后一位
        quantiles = quantiles[1:-1]
        loss = self.loss_function(f_x, y, quantiles)
        return loss # Assuming you want the mean loss across the batch

    def loss_function(self, f_x, y,c_k):
        loss = 0

        for c in c_k:
            # Calculate the sign function
            sgn = torch.where(y < c, -1, 1)
            # Apply the custom h function to the term
            term = sgn * (f_x - c)
            #防止exp溢出
            term_clamped = torch.clamp(term, min=-32, max=32)
            h_term = torch.log1p(torch.exp(-term_clamped))  # h(z) = log(1 + exp(-z))
            loss += h_term
        return loss.mean()   
