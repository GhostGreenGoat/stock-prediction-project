...
#%%
all_files_ridgeCV = [pd.read_csv(f'./ridgeCV/{f}',
...
data = "xy_data_old_3355"
label = 'y1_label'
label_normalization = ''
selected_feature = "all"
feature_nomalization = 'uniform'
limit = 'ud_limit_h2'
loss = "mse"
use_diff = False
if use_diff:
...
all_files_mse = [pd.read_csv(f'/home/laiminzhi/new_data/predict_data/{subframe}/{frame}/{f}',
                            dtype={'date':str})for f in sorted(os.listdir(f'/home/laiminzhi/new_data/predict_data/{subframe}/{frame}/'))]
#%%
all_files_lstm = [pd.read_csv(f"/home/laiminzhi/new_data/predict_data/lstm_30_y1_label__new_uniform_no_ud_limit_h2/selected_feature(new71)_loss(mse)_timestep(30)_layers(2)_hidden(128)_dropout(0.2)/{f}",
                              dtype={'date':str})for f in sorted(os.listdir(f'/home/laiminzhi/new_data/predict_data/lstm_30_y1_label__new_uniform_no_ud_limit_h2/selected_feature(new71)_loss(mse)_timestep(30)_layers(2)_hidden(128)_dropout(0.2)/'))]
#%%
...
for i,file in enumerate(all_files_ridgeCV):
    draw_pdf(file['Y_hat'],file['Y'])
    draw_scatter(file['Y_hat'],file['Y'])
    if i==10:
        break
#%%
draw_quantile(all_files_ridgeCV,object = 'Y')
#%%
correct_rate = []
mistake_rate = []
for file in all_files_ridgeCV:
    x=file
    ...
    correct_rate.append(quantile_y_true_ratio)
    ...
    mistake_rate.append(quantile_y_mistake_ratio)
...
x = range(len(y1))
plt.plot(x,(y1).to_list(),label="correct")
plt.plot(x,(y2).to_list(),label="mistake")
#把ldegend放到图外面
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)

plt.title("N(Y>90%)-N(Y<10%),N(Y_hat>90%)")
#%%
#把quantile中正确预测和错误预测的比例画出来
correct_rate =[]
mistake_rate = []
for file in all_files_lstm:
    x=file
    x['quantile_y'] = pd.qcut(file['Y'], 5, labels=range(1, 6))
    x['quantile_y_hat'] = pd.qcut(file['Y_hat'], 5, labels=range(1, 6))
    #x选出y_hat前10%的股票
    ...
    subset=x
    #查看subset中quantile_y也为quantile的比例
    quantile_y_true = subset[subset['Y'] > file['Y'].quantile(0.5) ]
    quantile_y_true_ratio = len(quantile_y_true)/len(subset)
    correct_rate.append(quantile_y_true_ratio)

    #查看subset中quantile_y为中位数以后的quantiel的比例
    quantile_y_mistake = subset[subset['Y'] < file['Y'].quantile(0.5) ]
    quantile_y_mistake_ratio = len(quantile_y_mistake)/len(subset)
    mistake_rate.append(quantile_y_mistake_ratio)
...
y2 = pd.Series(y2).rolling(30).mean()
x = range(len(y1))
plt.plot(x,y1,label='correct_rate')
#画虚线
plt.plot(x,y2,label='mistake_rate',linestyle='--')
#把ldegend放到图外面
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
plt.show()
# %%
...
yest = pd.concat(all_files_mse, axis=0) #贴合所有的预测值
XY = get_XY(yest)
alpha = cal_alpha(XY)
print(alpha)
#%%
