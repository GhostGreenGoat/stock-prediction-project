...
class Config:
    ...
    #规定选用的特征集合。all,am,pm分别表示全天，早盘，晚盘, new表示新的特征集合
    selected_feature = "new"
    selected_feature_set = pd.read_csv('selected_feature.csv',index_col=0)['AlphaName'].to_list()
    ...
    label_normalization = '' #取值有profile,rank,decentralization和空值
    feature_nomalization = 'uniform' #取值有uniform,normal,minmax,robust和空值

    #训练参数
    learning_rate = 0.001
    epoch = 44 #不考虑早停的前提下整个模型训练多少遍
    patience = 50
    batch_size = 1
    ...
    use_diff = False
    if use_diff:
        subframe = f"{data}_diff_{label}_{label_normalization}_{selected_feature}_{feature_nomalization}_no_{limit}"
    else:
        subframe = f"{data}_{label}_{label_normalization}_{selected_feature}_{feature_nomalization}_no_{limit}"
    if not use_dropout and not use_batch_norm:
        frame = f"selected_feature({selected_feature}{len(selected_feature_set)})_loss({loss})_depth{len(layer_sizes)}_sizes({size})"
    elif use_batch_norm and not use_dropout:
        frame = f"selected_feature({selected_feature}{len(selected_feature_set)})_loss({loss})_depth{len(layer_sizes)}_sizes({size})_batchnorm"
    elif use_dropout and not use_batch_norm:
        frame = f"selected_feature({selected_feature}{len(selected_feature_set)})_loss({loss})_depth{len(layer_sizes)}_sizes({size})_dropout"
    else:
        frame = f"selected_feature({selected_feature}{len(selected_feature_set)})_loss({loss})_depth{len(layer_sizes)}_sizes({size})_bd"
    #路径参数
    train_data_path = f"/home/laiminzhi/data/xydata/{data}.h5"
    diff_data_path = f"/home/laiminzhi/data/xydata/{data}_diff.h5"
    model_save_path = f"/home/laiminzhi/new_data/model/{subframe}/"
    figure_save_path = f"/home/laiminzhi/new_data/figure/{subframe}/"
    predict_save_path = f"/home/laiminzhi/new_data/predict_data/{subframe}/{frame}/"
    valid_save_path = f"/home/laiminzhi/new_data/predict_data/{subframe}/{frame}_valid/"
    processed_train_data_path = f"/home/laiminzhi/new_data/train_data/"+ subframe +"/"
    processed_test_data_path = f"/home/laiminzhi/new_data/test_data/"+ subframe+"/"
...
class Data:
    def __init__(self,config):
        ...
        #print("data:",self.data.shape,self.data.head())
    ...
    def read_data(self):
        ...
        if self.config.use_diff:
            data_diff = pd.read_hdf(config.diff_data_path)
            data_diff = data_diff.loc[:,selected_feature]
            #data_diff = data_diff.loc[:,data_diff.columns.str.startswith('x')]

            data_diff.columns = [col+"_diff" for col in data_diff.columns]
            init_data = pd.concat([init_data,data_diff],axis=1)
            data_column_name = selected_feature + data_diff.columns.to_list()
...
def load_train_data(config=config,path = config.processed_train_data_path):
    ...
    #从中选出需要用到的feature
    train_x = train_x.loc[:,config.selected_feature_set]
    valid_x = valid_x.loc[:,config.selected_feature_set]
...
def load_test_data(config=config,path=config.processed_test_data_path):
    ...
    test_x = test_x.loc[:,config.selected_feature_set]
    return test_x,test_y
...
if config.save_processed_train_data == True:
    ...
    xy = pd.read_hdf(config.train_data_path)
    print("Data loaded!")
